...
BasicObject::BasicObject(void) : BasicEntity()
{
	textureCoords = new FV2[6];
	textureId = new ULong();
	bitmapName = "No Bitmap";
	canAnimate = false;
}

BasicObject::BasicObject(ULong id, string name, string desc) : BasicEntity(id, name, desc)
{
	textureCoords = new FV2[6];
	textureId = new ULong();
	bitmapName = "No Bitmap";
...
BasicObject::BasicObject(ULong id, string name, string desc, 
						 string bitmap, Pos2D size, bool isAnimated) : 
						 BasicEntity(id, name, desc, size)
{
	textureId = new ULong();
	textureCoords = new FV2[6];
	bitmapName = bitmap;
